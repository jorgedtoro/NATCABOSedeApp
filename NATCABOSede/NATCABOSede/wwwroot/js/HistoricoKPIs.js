...
function loadPage(page) {
    const IdLinea = document.getElementById("lineaSeleccionada").value;
...
document.getElementById("btn-export-excel").addEventListener("click", function () {
    ...
    if (!desde || !hasta) {
        alert("Por favor, para exportar datos complete al menos la fecha de inicio y fin.");
        return;
    }
    ...
    fetch('/KPIS/Historico/ExportarExcel', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ IdLinea, confeccion,desde, hasta })
    })
...
function cargarDropdown(url, selectElement, placeholderText, formatOption) {
    fetch(url)
        ...
        .then(items => {
            ...
            // Clear options but KEEP the placeholder
            selectElement.innerHTML = '';

            // Create the placeholder option (always available)
            const placeholderOption = document.createElement('option');
            placeholderOption.value = '';  // Ensure it has a valid empty value
            placeholderOption.textContent = placeholderText;
            placeholderOption.disabled = false; // Allow it to be selected again
            placeholderOption.selected = true;  // Default selection
