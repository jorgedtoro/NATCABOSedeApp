...
function loadPage(page) {
    ...
    if (!desde || !hasta) {
        alert(
            'Por favor, introduzca al menos las fechas de inicio y fin antes de cambiar de página.'
        );
        return;
    }
...
function cargarDropdown(url, selectElement, placeholderText, formatOption) {
    fetch(url)
        ...
        .then((items) => {
            ...
            const uniqueItems = Array.from(new Set(items.map((item) => JSON.stringify(item)))).map(
